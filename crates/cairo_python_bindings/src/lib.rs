...
use cairo_sierra_to_casm::compiler::compile_at_path as compile_sierra_to_casm_at_path;
...
fn call_cairo_to_sierra_compiler(
    ...
    let sierra_program =
        compile_cairo_to_sierra_at_path(Path::new(input_path), CompilerConfig::default())
...
fn call_sierra_to_casm_compiler(
    ...
    if let Some(path) = output_path {
        fs::write(path, cairo_program_contents)
            .map_err(|_| PyErr::new::<RuntimeError, _>("Failed to write output."))?;
        return Ok(None);
    }
